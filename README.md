...
- **型安全性**: TypeScriptとZodによる厳密な型定義（`any`型・型アサーション禁止）
- **テスト駆動開発**: Vitestによる包括的なユニットテストと結合テスト
- **モジュラー設計**: 単一責任の原則に基づいた再利用可能なコンポーネント
- **自動スキーマ生成**: ZodスキーマからMCP用JSONスキーマを自動生成
...
- **ダメージ計算** (`calculate_damage`): 以下の要素を考慮した総合的なダメージ計算
  ...
  - もちもの効果（こだわりハチマキ、たつじんのおび、各種プレートなど）
...
1. **機能追加時**: テスト駆動開発（TDD）を基本とする
   - テストを先に書く（Red）
   - 最小限の実装でテストを通す（Green）
   - リファクタリング（Refactor）

2. **コード変更時**: `npm run check`でエラーがないことを確認

3. **スキーマ変更時**: Zodスキーマを変更した場合は必ず`npm run schemagen`を実行
